...
sets the pin where data comes in, i.e. Data Pin.
sets the module to begin readings. Required in the "on start" block.
...
3. The initializer method `begin(DT,SCK)` has a a built-in `set_gain(byte gain)` function that allows you to set the gain factor
...
Thanks to https://github.com/daferdur who created the HX711 library for Microbit version in 2019.
This repository forked from https://github.com/daferdur/pxt-myHX711, then modified for customized use.
